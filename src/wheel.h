 ...
 */
float getRampedRateForMotorDrive(float currentRate, float targetRate, float motorCurrent, float maxCurrent) {
  // Determine change in rate while allowing for ramping.
  float rateDelta = capRange(-MOTOR_RAMPING_DELTA, targetRate - currentRate, MOTOR_RAMPING_DELTA);
  float rate = currentRate + rateDelta;
  // Determine maximum drive rate while allowing for current limiting.
  if (abs(motorCurrent) > maxCurrent) { 
    float maxRate = currentRate * (maxCurrent / abs(motorCurrent));
...
class Wheel {
  private:
   ...
   /** Steering motor drive rate in the last step. */
    float steeringDriveRate = 0;
     /** Drive motor target drive rate in the last step. */
    float targetDriveRate = 0;
    ...
    void updateDriveMotor(float motorCurrent) {
      float rampedRate = getRampedRateForMotorDrive(
    ...
    void updateSteeringMotor(float rate, float motorCurrent) {
      float rampedRate = getRampedRateForMotorDrive(
  ...
  public:
    ...
    void update(float driveMotorCurrent, float steeringMotorCurrent) {
      uint32_t now = micros();
      ...
      float currentPosition = (float) this->encoder->read() / ENCODER_TICKS_PER_DEGREE;
      ...
      if (this->previousStepTime > 0 && fabs(this->steeringDriveRate) > STUCK_CHECK_DRIVE_RATE_THRESHOLD) {
        float actualMovement = normaliseValueToRange(-180, currentPosition - this->position, 180);
        uint32_t elapsedTimeMicroSec = now - this->previousStepTime; // Note: this handles wrap around of timestamp.
        float elapsedTimeSec = elapsedTimeMicroSec * 0.000001;
        float expectedMovement = NOMINAL_MAX_DEGREES_PER_SECOND * elapsedTimeSec * this->steeringDriveRate;
      ...
      if (!this->ready) {
        ...
        float error = this->targetPosition - this->position;
        ...
        float output = capRange(-1, error * STEERING_PID_K, 1);
        this->updateSteeringMotor(output, steeringMotorCurrent);
      }

      // Get current drive rate from the motor controller. Do this at the end to allow time for the set command
      // sent by updateDriveMotor() to take effect. Probably not necessary?
      this->driveRate = this->driveMotorController->get('M', this->driveMotorControllerChannel) / 2047.0;

      this->previousStepTime = now;
    }

    /** Set (target) drive rate, in range [-1, 1]. */
    void setDriveRate(float rate) { this->targetDriveRate = rate; }

    /** Get the current drive rate of the drive motor from the motor controller, in range [-1, 1]. */
    float getDriveRate() { return this->driveRate; }

    /** Set target position of the wheel in degrees, in range [-180, 180]. */
    void setWheelPositionTarget(float newTargetPosition) { this->targetPosition = newTargetPosition; }
    ...
    float getSteeringMotorDriveRate() { return this->steeringDriveRate; }

    /** Get the amount of time that the wheel has been stuck, in seconds. */
    float getStuckTime() { return this->stuckTime; }
