...
// so the count given by the encoder library will be float the PPR for every revolution of the encoder.
// The encoder is attached to the motor output, so factor in the gearing ratio of that to the actual wheel.
const int ENCODER_TICKS_PER_WHEEL_REVOLUTION = 2 * ENCODER_PPR * ((float) STEERING_SPROCKET_TEETH / (float) STEERING_PINION_TEETH); 
...
const float STEERING_PID_K = 0.5;

const int STEERING_PWM_FREQUENCY = 20000;

const float CURRENT_SENSOR_VOLTS_PER_AMP = 0.05 * (3.0 / 4.5); // Sensor output is 0.05v/A, 5v to 3.3v level shifting is via 1.5k and 3.0k resistors.

/** The expected nominal degrees per second turn rate at the maximum turn rate. */
const float NOMINAL_MAX_DEGREES_PER_SECOND = (37.48 / 60) * 360;

/** If a wheel has turned less than this proportion of the expected amount, consider the wheel stuck. */
const float STUCK_MOVEMENT_THRESHOLD = 0.3;

/** Only check if the wheel is stuck if the drive rate is greater than this. */ 
const float STUCK_CHECK_DRIVE_RATE_THRESHOLD = 0.1;

/** Frequency of wheel updates. */
const int WHEEL_UPDATE_FPS = 20;
/** Period of wheel updates, in us. */
const int WHEEL_UPDATE_PERIOD = 1000000 / WHEEL_UPDATE_FPS;

/** Time it takes in seconds to go from full stop to full speed for the drive and steering motors. */
const float MOTOR_RAMPING_TIME = 1;
/** The maximum percentile change in motor drive rate per update step. */
const float MOTOR_RAMPING_DELTA = 1.0 / (MOTOR_RAMPING_TIME * WHEEL_UPDATE_FPS);

/** The maximum current draw allowed for drive motors, in amps. */
const float DRIVE_MOTOR_MAX_CURRENT = 30;
/** The maximum current draw allowed for drive motors, in amps. */
const float STEERING_MOTOR_MAX_CURRENT = 18;
